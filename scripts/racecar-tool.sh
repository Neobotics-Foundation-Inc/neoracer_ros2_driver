...
lab dashboards:  camlabel (8082), wallfollow (8081), pursuit (8083),
                 eps (8084), smartfollow (8085), linefollow (8086)
            ...
            if [[ $COMP_CWORD -eq 2 ]]; then
                ...
                case "$action" in
                    start|stop|restart|logs|enable|disable)
                        COMPREPLY=( $(compgen -W "teleop watchdog dashboard jupyter camlabel wallfollow pursuit eps smartfollow linefollow" -- "$cur") )
                        ;;
                esac
            fi
            ;;
